export default {
    ...
    backup: {
      ...
      conversationCapture: {
        ...
        scopeEssential: 'Essential (recommended): this turn\'s user input + model output',
        scopeFull: 'Full: the entire redacted request',
        scopeEssentialHint: 'Drops system prompts, tool definitions, injected context and resent history. Agent clients resend the whole conversation every turn, so storing it in full copies the same content N times; on production samples this cuts a record from 1.19MB to 5.7KB.',
        scopeFullHint: 'Keeps the whole request including system prompts and tool history. Only needed for agent-trajectory distillation; roughly 200x larger per record on production samples.',
