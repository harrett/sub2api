export default {
    ...
    backup: {
      ...
      conversationCapture: {
        ...
        prefix: 'Key 前缀',
        scope: '留存范围',
        scopeEssential: '精简（推荐）：只留本轮用户输入 + 模型输出',
        scopeFull: '完整：保留脱敏后的整个请求',
        scopeEssentialHint: '丢弃系统提示、工具定义、注入上下文和重发的历史。agent 客户端每轮都会重发整段对话，存全量等于同一份内容抄 N 遍；生产抽样上精简后单条从 1.19MB 降到 5.7KB。',
        scopeFullHint: '保留整个请求（含系统提示与工具历史），仅在需要做 agent 轨迹蒸馏时开启。生产抽样上单条约为精简模式的 200 倍。',
