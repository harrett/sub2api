<template>
  <AppLayout>
    <div class="custom-purchase-page mx-auto max-w-[1500px] space-y-6">
      <div v-if="loading" class="flex items-center justify-center py-20">
        <div class="h-8 w-8 animate-spin rounded-full border-4 border-primary-500 border-t-transparent"></div>
      </div>
      <template v-else>
        <!-- Tab Switcher (hide during payment and subscription confirm) -->
        <div v-if="tabs.length > 1 && paymentPhase === 'select' && !selectedPlan" class="purchase-mode-tabs">
          <button v-for="tab in tabs" :key="tab.key"
            class="purchase-mode-tab"
            :class="{ 'purchase-mode-tab--active': activeTab === tab.key }"
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <template v-if="selectedPlan">
              <div class="card p-5">
                ...
                <!-- Purchase summary only shows the limits customers need to compare. -->
                <div class="mt-5 grid grid-cols-2 gap-3">
                  ...
                  <div v-if="selectedPlan.daily_limit_usd == null && selectedPlan.monthly_limit_usd == null">
                  ...
                  <div v-for="sub in activeSubscriptions" :key="sub.id"
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="flex flex-wrap gap-x-3 text-[11px] text-gray-400 dark:text-gray-500">
                        <span v-if="sub.group?.daily_limit_usd == null && sub.group?.monthly_limit_usd == null">{{ t('payment.planCard.quota') }}: {{ t('payment.planCard.unlimited') }}</span>
      ...
      <Transition name="modal">
        <div v-if="showRenewalModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" @click.self="closeRenewalModal">
          <div class="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-700 dark:bg-dark-900">
            ...
            <div class="space-y-4">
              <CustomSubscriptionPlanCard v-for="plan in renewalPlans" :key="plan.id" :plan="plan" :active-subscriptions="activeSubscriptions" @select="selectPlanFromModal" />
            </div>
          </div>
        </div>
      </Transition>
...
import CustomSubscriptionPlanCard from '@/components/payment/CustomSubscriptionPlanCard.vue'
...
// The custom purchase screen keeps plans in a horizontal comparison row.
const planGridClass = computed(() => {
  return 'purchase-plan-grid'
})
...
.purchase-mode-tab:hover { color: var(--text); }
...
.custom-purchase-page :deep(.purchase-plan-card) { height: 100%; }

@media (max-width: 639px) {
  .purchase-mode-tabs { gap: 2px; padding: 3px; }
  .purchase-mode-tab { min-height: 40px; padding-inline: 10px; font-size: 14px; }
  .purchase-plan-grid { grid-template-columns: minmax(0, 1fr); gap: 14px; }
}
</style>
