...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	backupCredentials := convlog.ProvideBackupCredentials(backupService)
	settingStore := convlog.NewSettingStore(settingRepository, secretEncryptor, backupCredentials)
	convlogRepository := convlog.NewRepository(db)
	objectStoreFactory := provideConvLogObjectStoreFactory()
	convlogService := convlog.NewService(settingStore, convlogRepository, objectStoreFactory)
	adminHandler := convlog.NewAdminHandler(convlogService)
	...
	bundleSubscriptionService := service.ProvideBundleSubscriptionService(client, settingService)
	bundleSubscriptionHandler := handler.ProvideBundleSubscriptionHandler(bundleSubscriptionService, paymentService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, bundleSubscriptionHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	optionalJWTAuthMiddleware := middleware.NewOptionalJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService, auditLogService)
	bundleEntitlementResolver := service.ProvideBundleEntitlementResolver(client, settingService)
	apiKeyAuthMiddleware := middleware.ProvideAPIKeyAuthMiddleware(apiKeyService, subscriptionService, bundleEntitlementResolver, configConfig)
