...
func TestConcurrencyErrorSplitsUserAndServerResponsibility(t *testing.T) {
	...
	userStatus, _, _, userMessage := concurrencyErrorResponse(&ConcurrencyError{SlotType: "user"}, "user")
	...
	accountStatus, _, _, accountMessage := concurrencyErrorResponse(&ConcurrencyError{SlotType: "account"}, "account")
...
func TestConcurrencyErrorSlotTypeOnErrorWinsOverArgument(t *testing.T) {
	...
	_, _, _, message := concurrencyErrorResponse(&ConcurrencyError{SlotType: "account"}, "user")
...
func TestConcurrencyErrorWaitQueueFullIsServerSide(t *testing.T) {
	status, errType, _, message := concurrencyErrorResponse(&WaitQueueFullError{SlotType: "user"}, "user")
...
func TestConcurrencyErrorClientDisconnectStaysUntouched(t *testing.T) {
	...
	status, errType, _, message := concurrencyErrorResponse(context.Canceled, "user")
...
func TestConcurrencyErrorUnknownFallbackIsServerSide(t *testing.T) {
	status, _, _, message := concurrencyErrorResponse(errors.New("redis down"), "user")
