...
func usageRecordContext(parent context.Context, base context.Context) context.Context {
	...
	if bundleBilling, ok := service.BundleUsageBillingFromContext(parent); ok {
		base = service.WithBundleUsageBilling(base, bundleBilling.Entitlement, bundleBilling.Resolver)
	}
	return base
}
