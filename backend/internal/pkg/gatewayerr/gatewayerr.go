...
var platformErrorGuidance = map[string]errorGuidance{
	...
	// 命中渠道「限制模型」（模型定价表白名单）时的调度器快路径拦截，与上面那条
	// 语义不同：不是没账号支持，是管理员主动把这个模型排除在渠道定价表之外。
	// 同样不可重试，但措辞要明确"这是管理员的策略选择"，不能说"重试无法解决"
	// 这种听起来像临时故障的话，否则用户会去反复重试或联系管理员报障。
	"MODEL_BLOCKED_BY_CHANNEL_POLICY": {
		label: "服务器侧问题",
		hint:  "该模型已被管理员在渠道设置中禁止使用，这是主动的策略限制，不是临时故障，重试无效。请改用其他模型。",
	},
