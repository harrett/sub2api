...
func TestCompatibleImagesGeminiModels(t *testing.T) {
	...
	// 前缀门的判定在本 fork 里推迟到选中账号之后（账号 model_mapping 可以显式声明
	// 第三方图像模型名，解析阶段无从判定），因此非图像模型在 parse 处只被记录，
	// 由 ValidateOpenAIImagesAccountModel 拒绝。
	for _, model := range []string{"gemini-2.5-pro", "gemini-2.5-flash", "gemini-3-pro-imageless", "unknown-image", "gpt-5.5"} {
		t.Run("reject_"+model, func(t *testing.T) {
			...
			parsed, err := (&OpenAIGatewayService{}).ParseOpenAIImagesRequest(c, body)
			require.NoError(t, err)
			require.ErrorContains(t, parsed.modelValidationErr, "images endpoint requires an image model")
			for _, typ := range []string{AccountTypeAPIKey, AccountTypeOAuth, AccountTypeSetupToken} {
				account := &Account{Platform: PlatformOpenAI, Type: typ}
