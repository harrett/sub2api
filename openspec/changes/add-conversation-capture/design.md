...
失败且模型什么都没输出的请求直接丢弃，只计数（运行态 `skipped_no_output_total`）。

生产一小时实测的依据：847 条记录里 608 条是 503/429/404，**无一条带模型输出**，
其中 596 条根本没绑定到上游账号——既没有可蒸馏的输出，也不构成账号封禁风险，
而客户端重试会把同一份输入反复写进来（最极端的一例：724KB 的输入被 503 重试
存了 5 份）。这类记录占了那一小时 79% 的字节。

失败但上游确实回了内容的请求仍然留存：那次调用真的用掉了账号。
...
响应侧只留元信息（`revised_prompt`、`url`、`size`）。`b64_json` 是整张图片，
单条可达 MB 级，已列入落盘前整段删除的字段，任何路径都不会写进对象存储。
